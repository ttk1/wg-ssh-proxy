...
- **完全 split-tunnel**: このプロセスが Dial した SSH の通信だけがトンネルを通る。
  他の通信には一切影響しない。
- **直接依存は wireguard-go と golang.org/x/crypto の 2 つ**(x/crypto は
...
(典型的には Windows クライアント)では本ツールのサブコマンドで代用できる。
`genkey` / `pubkey` / `genpsk` は wg(8) の同名コマンドと互換
(32 バイト鍵の base64、1 行)で、`wg` が作った鍵と相互に使える。

```powershell
# PowerShell の例。> リダイレクトは使わない (PowerShell 5.1 では UTF-16 で
# 書かれてしまう)。Set-Content / Get-Content のペアなら往復できる。
...
- `PresharedKey` の併用を推奨(「今記録して将来解読する」型の攻撃の緩和策になる)。
...
- **秘密鍵はファイル経由でのみ受け取る**(セットアップ 2 参照)。コマンドライン
  ...
  (`pubkey` サブコマンドも `wg pubkey` と同様に標準入力から受け取る。
  引数で渡す形にすると鍵がプロセス一覧に露出するため)。
- **AllowedIPs は Target の 1 ホストに限定**。WireGuard の cryptokey routing
  により、サーバ側が侵害されてもトンネル内からクライアント側の netstack へ注入できる
...
- **同じ設定(= 同じ鍵)での同時接続は 1 本に保つこと**。接続ごとに独立した
  WireGuard トンネルができるが、サーバから見ればどれも同一 peer なので、
  複数同時に張るとセッションを取り合い、互いに断続的に十数秒フリーズする。
  SFTP ファイルブラウザ等のために裏で 2 本目の接続を張る機能のある
  クライアントツールでは、その機能を無効にして使うこと。
